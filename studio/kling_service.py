...
def get_kling_headers():
    token = os.environ.get("KLING_API_TOKEN")
    return {
...
def generate_image2video(image_path_or_url, prompt, duration="5", mode="pro"):
    """
    ...
    response = requests.post(url, headers=get_kling_headers(), json=payload)
    response.raise_for_status()
    return response.json()

def generate_motion_control(image_path_or_url, video_url, prompt=""):
    """
...
def check_task_status(task_id, task_type="image2video"):
    """
    Polls the task status for either image2video or motion-control
    """
    url = f"{KLING_API_BASE}/{task_type}/{task_id}"
    response = requests.get(url, headers=get_kling_headers())
    response.raise_for_status()
    return response.json()
